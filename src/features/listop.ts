...
export const indentSelecttedBlock = (
    editor: Editor,
    ...
    if (direction === "indent") {
        if (level === undefined) {
            ...
            if (!isSelectSingleLine)
                newHead = { line: head.line, ch: head.ch + 2 };
        } else {
            newAnchor = { line: anchor.line, ch: anchor.ch + indent.length };
            if (!isSelectSingleLine)
                newHead = { line: head.line, ch: head.ch + indent.length };
        }
    } else {
        if (level === 0) {
            newAnchor = { line: anchor.line, ch: anchor.ch - prefix.length };
            if (!isSelectSingleLine)
                newHead = { line: head.line, ch: head.ch - prefix.length };
        } else {
            newAnchor = { line: anchor.line, ch: anchor.ch - indent.length };
            if (!isSelectSingleLine)
                newHead = { line: head.line, ch: head.ch - indent.length };
...
export const moveListBlock = (editor: Editor, to: "up" | "down") => {
    ...
    const src = editor.getRange(
        { line: srcBegin, ch: 0 },
        { line: srcEnd, ch: srcEndLength }
    );
    const dst = editor.getRange(
        { line: dstBegin, ch: 0 },
        { line: dstEnd, ch: dstEndLength }
    );
